...
// Tabla de posiciones de renta variable (equity + ETF de renta variable)
export default function EquityTable({ analytics }) {
    ...
    const filtered = positions.filter(p => (p.asset_type ?? 'equity') === 'equity');
    ...
    if (filtered.length === 0) {
        return (
            <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
                Sin posiciones de renta variable.
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <SortButton id="yield"  label="Yield/Dividendo" current={sortBy} onClick={setSortBy} />
            ...
            <div style={{ marginTop: '0.75rem', fontSize: '0.77rem', color: 'var(--text-secondary)' }}>
                * P&L Real = precio actual vs precio medio ponderado de compra ·
                <AlertTriangle size={10} style={{ display: 'inline', color: 'var(--danger)', margin: '0 0.2rem' }} />
                = sobrepeso {'>'} 15% / deterioro {'>'} -15%
            </div>
        </div>
    );
}
